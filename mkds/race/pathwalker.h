...
void pw_init(pw_pathwalker_t* pw, u16 initialPoint, bool32 forwards);
bool32 pw_update(pw_pathwalker_t* pw);
...
static inline bool32 pw_hasEnded(pw_pathwalker_t* pw)
{
	return !pw->path->loop &&
...
{
	pw->speed = speed;
	pw->partSpeed = fxMulT(speed, pw->path->parts[pw->partIdx].oneDivLength);
}
